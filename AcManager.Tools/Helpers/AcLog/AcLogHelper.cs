...
namespace AcManager.Tools.Helpers.AcLog {
    public static class AcLogHelper {
        ...
        public static WhatsGoingOn TryToDetermineWhatsGoingOn() {
            try {
                return DetermineWhatsGoingOn(File.Open(AcPaths.GetLogFilename(), FileMode.Open, FileAccess.ReadWrite, FileShare.ReadWrite)
                        .ReadAsStringAndDispose());
            } catch (Exception e) {
        ...
        [CanBeNull]
        public static WhatsGoingOn DetermineWhatsGoingOn(string log) {
            {
...

            {
                ...
                if (i != -1) {
                    ...
                    var blurredIndex = error.IndexOf(@"BLURRED NODE (", StringComparison.Ordinal);
                    if (blurredIndex != -1 && error.Contains(@": CANNOT LOCATE OBJECT", StringComparison.Ordinal)) {
                        var blurredIndex2 = error.IndexOf(@") ", blurredIndex, StringComparison.Ordinal);
                        if (blurredIndex2 != -1) {
                            var nodeName = error.Substring(blurredIndex + @"BLURRED NODE (".Length,
                                    blurredIndex2 - (blurredIndex + @"BLURRED NODE (".Length));
                            return new WhatsGoingOn(WhatsGoingOnType.IncorrectBlurredObjects, nodeName);
                        }
                    }
...

            {
                ...
                if (i != -1) {
                    ...
                    if (crash.Contains(@"RotatingObjects::update")) {
                        return new WhatsGoingOn(WhatsGoingOnType.IncorrectRotatingObjects);
                    }
