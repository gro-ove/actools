...
namespace AcManager.Tools.AcManagersNew {
    ...
    public abstract partial class AcManagerNew<T> : IDirectoryListener, IWatchingChangeApplier where T : AcCommonObject {
        ...
        void IDirectoryListener.FileOrDirectoryChanged(object sender, FileSystemEventArgs e) {
            ...
            if (objectLocation == null || !Filter(objectLocation) || inner && ShouldSkipFile(e.FullPath)) return;
        ...
        void IDirectoryListener.FileOrDirectoryRenamed(object sender, RenamedEventArgs e) {
            ...
            if (inner) {
                GetWatchingTask(objectLocation).AddEvent(WatcherChangeTypes.Changed, null, ShouldSkipFile(e.OldFullPath) ? e.FullPath : null);
            } else {
        ...
        protected abstract bool ShouldSkipFileInternal(string filename);

        public bool ShouldSkipFile(string filename) {
            return filename.EndsWith(".tmp", StringComparison.OrdinalIgnoreCase) || ShouldSkipFileInternal(filename);
        }
    }
}
