...
namespace AcManager.Pages.SelectionLists {
    public partial class TrackDlcs {
        public TrackDlcs() : base(TracksManager.Instance, true) {
            InitializeComponent();
        }
        ...
        protected override void AddNewIfMissing(IList<SelectDlc> list, TrackObject obj) {
            var value = obj.Dlc ?? (obj.Author == AcCommonObject.AuthorKunos ? FakeKunosDlc : FakeModDlc);
            for (var i = list.Count - 1; i >= 0; i--) {
                var item = list[i];
